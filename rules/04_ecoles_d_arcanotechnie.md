# Écoles d'Arcanotechnie

L'arcanotechnie est une **énergie potentielle fondamentale**, présente dans toute matière. Les pratiquants — appelés _canalisateurs_ — apprennent à convertir cette énergie en différentes formes physiques mesurables : chaleur, lumière, champ électromagnétique, gravité, etc.
Chaque école correspond à une **conversion spécifique** d'énergie arcanotechnique en énergie physique, biologique ou informationnelle.

---

## 🔥 Feu — Conversion thermique rapide

**Principe :** Conversion directe du mana en énergie cinétique moléculaire, provoquant la combustion ou l’explosion.

**Forces :**

- Dégâts instantanés très élevés.
- Attaques de zone (AOE).
- Dégâts sur la durée (brûlures).

**Faiblesses :**

- Consommation de mana importante.
- Faible contrôle.
- Inefficace en atmosphère pauvre en oxygène.

---

## ❄️ Givre — Extraction énergétique

**Principe :** Retrait de l’énergie cinétique ambiante, créant un gradient thermique négatif.

**Forces :**

- Contrôle de zone (ralentissement, immobilisation).
- Excellent rendement énergétique.
- Peut renforcer la défense par refroidissement local.

**Faiblesses :**

- Dégâts faibles.
- Lente à agir.
- Fragile en environnement chaud.

---

## ⚡ Électricité — Conversion électromagnétique

**Principe :** Transformation du mana en champ électromagnétique ordonné (éclairs, décharges).

**Forces :**

- Vitesse d’activation quasi instantanée.
- Dégâts en chaîne entre cibles.
- Synergie avec la technologie.

**Faiblesses :**

- Risque de rebond sur alliés.
- Sensible à la mise à la terre.
- Consommation élevée en environnement humide.

---

## 💢 Kinesthésique — Conversion gravito-mécanique

**Principe :** Transformation du mana en travail mécanique (poussée, pression, gravité locale).

**Forces :**

- Contrôle du terrain.
- Affecte toute matière.
- Polyvalente entre attaque et défense.

**Faiblesses :**

- Très coûteuse selon la masse des cibles.
- Risque de déformation de matière.
- Réaction lente dans les environnements instables.

---

## ☀️ Lumière — Conversion photonique stabilisée

**Principe :** Conversion du mana en flux photonique cohérent contenu dans un champ photoarcanotechnique.
Permet la création de **murs de lumière solide** via confinement et rétro-alimentation énergétique.

**Forces :**

- Barrières stables et durables (mur de lumière).
- Auto-stabilisation partielle : faible consommation d’entretien.
- Capable de réfléchir ou dissiper énergie et projectiles.

**Faiblesses :**

- Consommation initiale élevée.
- Vulnérable aux interférences arcanotechniques (ombre, pure).
- Déphase sous surcharge énergétique.

---

## 🌑 Ombre — Manipulation de l’information

**Principe :** Modifie la propagation des ondes lumineuses et sonores, agissant sur la perception plutôt que la matière.

**Forces :**

- Furtivité totale.
- Contrôle mental léger (confusion, peur).
- Annule ou détourne la détection optique/sonore.

**Faiblesses :**

- Aucun effet physique réel.
- Inefficace contre IA et machines.
- Dissipée instantanément par lumière pure.

---

## 💀 Biométabolique — Conversion bioénergétique

**Principe :** Manipule le champ vital des organismes : extraction, transfert ou inversion de l’énergie biologique.

**Forces :**

- Drain de vie, vol d’énergie.
- Invocation de serviteurs.
- Auto-entretien via absorption vitale.

**Faiblesses :**

- Instabilité mentale de l’utilisateur.
- Corruption progressive.
- Contrecarrée par l'arcanotechnie de lumière.

---

## 💫 Quantique — Manipulation quantique

**Principe :** Altération des liaisons subatomiques et états de matière (spin, charge, phase).

**Forces :**

- Téléportation, transmutation, altération de densité.
- Interaction universelle avec toute autre arcanotechnie.
- Annule les effets physiques sans destruction.

**Faiblesses :**

- Hautement instable.
- Difficile à maîtriser.
- Consommation variable et imprévisible.

---

## ⚪ Pure — Manipulation du flux arcanotechnique

**Principe :** Action sur l'arcanotechnie elle-même : interruption, déviation, brûlure de mana, amplification.

**Forces :**

- Contre toute autre arcanotechnie.
- Neutralisation de sort.
- Annulation de flux arcanotechnique.

**Faiblesses :**

- Aucun effet sur la matière.
- Complexité extrême.
- Nécessite maîtrise complète du flux global.

---

## 🌪️ Aérocinétique — Conversion atmosphérique

**Principe :** Conversion du mana en énergie mécanique macroscopique (vent, pression).

**Forces :**

- Contrôle de zone.
- Déviation de projectiles.
- Perturbe les tirs ennemis.

**Faiblesses :**

- Inefficace dans le vide.
- Peu précis.

---

## 🌊 Hydrodynamique — Conversion fluide

**Principe :** Transformation du mana en énergie contrôlant l’eau et ses états.

**Forces :**

- Contrôle de terrain.
- Polyvalente avec givre et feu.
- Peut soutenir les alliés (refroidissement, purification).

**Faiblesses :**

- Nécessite source d’eau.
- Lent en milieu sec.

---

## 🧲 Magnétique — Conversion magnétique

**Principe :** Génération de champs magnétiques puissants via le mana.

**Forces :**

- Contrôle du métal et blindage.
- Neutralisation de technologie.
- Lévitation magnétique.

**Faiblesses :**

- Inefficace sur matériaux non ferreux.
- Instabilité de champ à haute intensité.

---

## 🧬 Bioarcanotechnie — Conversion biologique

**Principe :** Utilisation du mana pour influencer la croissance et la régénération cellulaire.

**Forces :**

- Guérison, mutation, adaptation.
- Très faible coût énergétique.

**Faiblesses :**

- Risque de mutation incontrôlée.
- Effets imprévisibles.

---

## 🔊 Sonique — Conversion vibratoire

**Principe :** Transformation du mana en ondes acoustiques ou sismiques.

**Forces :**

- Dégâts structurels.
- Contournement des armures.
- Désorientation des ennemis.

**Faiblesses :**

- Inefficace dans le vide.
- Peu sélectif.

---

## 🕰️ Chronodynamique — Conversion temporelle

**Principe :** Manipulation du rythme local du temps via compression ou dilatation du flux de mana.

**Forces :**

- Altération du temps perçu.
- Stabilisation ou désynchronisation des flux.

**Faiblesses :**

- Risque de distorsion temporelle.
- Contre-effet cumulatif (rebond temporel).

---

## 🌌 Gravitonique — Conversion gravitationnelle

**Principe :** Altération du champ gravitationnel local à l’aide du mana.

**Forces :**

- Contrôle du mouvement et du poids.
- Défense anti-projectile.

**Faiblesses :**

- Instabilité du champ.
- Risque d’implosion.

---

## 🪞 Illusion — Manipulation de la perception

**Principe :** Transformation du mana en signaux informationnels altérant la perception sensorielle.

**Forces :**

- Manipule les sens et capteurs.
- Peu coûteuse.

**Faiblesses :**

- Inefficace contre IA.
- Dissipée par l'arcanotechnie pure.

---

## ⚔️ Martial — Techniques de combat physique

**Principe :** Techniques de combat rapproché développées par l'entraînement et l'expérience, n'impliquant aucune manipulation arcanotechnique. Basées sur la maîtrise du corps, des armes de mêlée et de la stratégie.

**Forces :**

- Aucune consommation de mana.
- Fonctionne en toutes circonstances.
- Améliore les capacités d'armement de mêlée.

**Faiblesses :**

- Limitées par les capacités physiques.
- Nécessitent un entraînement long et rigoureux.
- Inefficaces contre certaines protections arcanotechniques.

---

## 🎯 Balistique — Techniques de combat à distance

**Principe :** Techniques de tir et de combat à distance développées par l'entraînement spécialisé avec les armes de tir. Maîtrise de la précision, de la cadence et des munitions spécialisées.

**Forces :**

- Aucune consommation de mana.
- Efficace à longue portée.
- Améliore les capacités d'armement à distance.
- Permet l'usage de munitions spécialisées.

**Faiblesses :**

- Dépendante des munitions et de l'équipement.
- Nécessite entraînement spécialisé au tir.
- Vulnérable en combat rapproché.

---
